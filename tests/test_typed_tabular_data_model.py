...
from jpy_tools.typed_tabular_data_model import TypedTabularDataModel, TypeConfig
...
class TestTypedTabularDataModel(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.custom_configs = {
            ...
            DataType.DATE: TypeConfig(empty_default=date(1900, 1, 1), none_default=None),
            DataType.DATETIME: TypeConfig(empty_default=datetime(1900, 1, 1), none_default=None)
        }
        
        # Create models with default and custom configurations
        self.default_model = TypedTabularDataModel(self.test_data)
        self.custom_model = TypedTabularDataModel(self.test_data, type_configs=self.custom_configs)
    ...
    def test_column_types(self):
        ...
        self.assertEqual(self.default_model.column_type("created_at"), DataType.DATETIME)
    ...
    def test_default_conversions(self):
        ...
        self.assertEqual(self.default_model.cell_value("birth_date", 0), date(1998, 1, 1))
        self.assertEqual(self.default_model.cell_value("created_at", 0), datetime(2024, 1, 1, 12, 0))
        
        # Test empty values
        self.assertEqual(self.default_model.cell_value("age", 1), 0)  # empty_default for INTEGER
        self.assertEqual(self.default_model.cell_value("score", 1), 0.0)  # empty_default for FLOAT
        self.assertEqual(self.default_model.cell_value("is_active", 1), False)  # empty_default for BOOLEAN
        
        # Test none-like values
        self.assertEqual(self.default_model.cell_value("age", 2), 0)  # none_default for INTEGER
        self.assertEqual(self.default_model.cell_value("score", 2), 0.0)  # none_default for FLOAT
        self.assertEqual(self.default_model.cell_value("is_active", 2), False)  # none_default for BOOLEAN
    ...
    def test_custom_conversions(self):
        ...
        self.assertEqual(self.custom_model.cell_value("birth_date", 0), date(1998, 1, 1))
        self.assertEqual(self.custom_model.cell_value("created_at", 0), datetime(2024, 1, 1, 12, 0))
        
        # Test empty values with custom defaults
        self.assertEqual(self.custom_model.cell_value("age", 1), -1)  # custom empty_default for INTEGER
        self.assertEqual(self.custom_model.cell_value("score", 1), 0.0)  # custom empty_default for FLOAT
        self.assertEqual(self.custom_model.cell_value("is_active", 1), False)  # custom empty_default for BOOLEAN
        self.assertEqual(self.custom_model.cell_value("birth_date", 1), date(1993, 5, 15))  # actual date value
        
        # Test none-like values with custom defaults
        self.assertEqual(self.custom_model.cell_value("age", 2), 0)  # custom none_default for INTEGER
        self.assertEqual(self.custom_model.cell_value("score", 2), 0.0)  # custom none_default for FLOAT
        self.assertEqual(self.custom_model.cell_value("is_active", 2), True)  # custom none_default for BOOLEAN
        self.assertEqual(self.custom_model.cell_value("birth_date", 2), None)  # custom none_default for DATE
    ...
    def test_mixed_type_handling(self):
        ...
        self.assertEqual(mixed_model.cell_value("mixed_col", 0), "123")  # integer-like preserved as string
        self.assertEqual(mixed_model.cell_value("mixed_col", 1), "abc")  # string preserved
        self.assertEqual(mixed_model.cell_value("mixed_col", 2), "")  # empty value preserved
        self.assertEqual(mixed_model.cell_value("mixed_col", 3), "NONE")  # none-like uses none_default
