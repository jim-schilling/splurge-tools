...
def workflow_1_data_cleaning_and_validation(files):
    ...
    print("=== Workflow 1: Data Cleaning and Validation ===\n")
    
    print("Step 1: Parse messy CSV data")
    raw_data = DsvHelper.parse_file(files['messy'], delimiter=",", bookend='"')
    print(f"  Loaded {len(raw_data)} rows (including header)")
    print(f"  Sample raw row: {raw_data[1]}")
    print()
    
    print("Step 2: Text normalization and cleaning")
    ...
    print(f"  Sample cleaned row: {cleaned_data[1]}")
    print()
    
    print("Step 3: Create tabular data model")
    model = TabularDataModel(cleaned_data, header_rows=1)
    print(f"  Created model: {model.row_count} rows × {model.column_count} columns")
    ...
    print("Step 5: Data validation with business rules")
    ...
    validator.add_validator("Active", lambda x: x.lower() in ["true", "false", "yes", "no", "1", "0", "y", "n", "active"])
    ...
    print(f"  Validation results: {len(valid_rows)} valid, {len(invalid_rows)} invalid")
    
    # Show invalid records
    if invalid_rows:
        print("  Invalid records:")
        for i, record, errors in invalid_rows[:3]:  # Show first 3
            print(f"    Row {i}: {record.get('Name', 'Unknown')} - {errors}")
    print()
    
    print("Step 6: Create clean dataset")
    if valid_rows:
...
def workflow_2_streaming_data_analysis(files):
    ...
    print("=== Workflow 2: Streaming Data Analysis ===\n")
    
    print("Step 1: Set up streaming data processing")
    print(f"  Processing large file: {files['large']}")
    print(f"  File size: {files['large'].stat().st_size / 1024 / 1024:.1f} MB")
    ...
    print(f"  Streaming model columns: {streaming_model.column_names}")
    print()
    
    print("Step 2: Streaming data analysis")
    ...
    print("Step 3: Generate streaming analytics report")
    if total_records > 0:
        avg_price = price_sum / total_records
        avg_quantity = quantity_sum / total_records
        avg_revenue_per_order = total_revenue / total_records
...
def workflow_3_data_transformation_pipeline():
    ...
    print("=== Workflow 3: Data Transformation Pipeline ===\n")
    
    print("Step 1: Generate sample sales data")
    ...
    print(f"  Created sales model: {model.row_count} rows × {model.column_count} columns")
    ...
    print("Step 2: Data transformation operations")
    transformer = DataTransformer(model)
    
    # Pivot by salesperson and product
    print("  Pivot transformation (Salesperson by Product):")
    try:
    ...
    # Group by region
    print("  Group by transformation (by Region):")
    try:
    ...
    print("Step 3: Column transformations")
    try:
        ...
        print(f"  Added profit column: {transformed.column_count} columns")
        print("  Sample with profit:")
        for i in range(min(3, transformed.row_count)):
            row = transformed.row(i)
            print(f"    Revenue: ${row['Revenue']}, Profit: ${row['Profit']}")
    except Exception as e:
...
def workflow_4_comprehensive_etl_pipeline(files):
    ...
    print("=== Workflow 4: Comprehensive ETL Pipeline ===\n")
    
    print("EXTRACT Phase")
    print("Step 1: Extract data from multiple sources")
    
    # Extract from messy CSV
    print("  Extracting from messy employee data...")
    ...
    print("  Generating supplementary department data...")
    dept_data = [
    ...
    print(f"    Generated {len(dept_data) - 1} department records")
    print()
    
    print("TRANSFORM Phase")
    print("Step 2: Data cleaning and normalization")
    ...
    for i, row in enumerate(employee_data):
        if i == 0:  # Header
            cleaned_employee_data.append([
                "Name", "Age", "Department", "Salary", "Email", "Start_Date", "Active"
            ])
            continue
        ...
        for j, cell in enumerate(row):
            ...
            if j == 0:  # Name
                ...
                clean_cell = clean_cell.lower()
                if clean_cell in ['true', 'yes', '1', 'y', 'active']:
                    clean_cell = 'true'
                else:
                    clean_cell = 'false'
    ...
    print(f"    Validated: {len(valid_employees)} valid employee records")
    
    print("Step 4: Data enrichment and joining")
    ...
    print(f"    Enriched: {len(enriched_employees)} records with department data")
    print()
    
    print("Step 5: Data aggregation and analytics")
    ...
    for dept, stats in dept_stats.items():
        ...
        active_rate = (stats["active_count"] / stats["count"]) * 100
        print(f"      {dept}:")
    ...
    print("LOAD Phase")
    print("Step 6: Prepare final datasets for loading")
    
    # Create final enriched dataset
    final_headers = [
        "Name", "Age", "Department", "Salary", "Email", "Start_Date", 
        "Active", "Manager", "Budget", "Location"
    ]
    ...
    print(f"    Final dataset: {final_model.row_count} rows × {final_model.column_count} columns")
    print("    Sample final records:")
    for i in range(min(3, final_model.row_count)):
        print(f"      {final_model.row(i)}")
...
def workflow_5_real_time_data_processing():
    ...
    print("=== Workflow 5: Real-time Data Processing Simulation ===\n")
    
    print("Step 1: Simulate incoming data stream")
    ...
    print(f"  Generated {len(incoming_records)} incoming records")
    print(f"  Sample record: {incoming_records[0]}")
    print()
    
    print("Step 2: Real-time validation and filtering")
    ...
    print(f"  Processed: {len(valid_records)} valid, {invalid_count} invalid")
    print()
    
    print("Step 3: Real-time analytics")
    ...
    print(f"    Total Events: {len(valid_records)}")
    print(f"    Success Rate: {(success_count/len(valid_records)*100):.1f}%")
    print(f"    Total Value: ${total_value:,.2f}")
    print(f"    Average Value: ${total_value/len(valid_records):.2f}")
    ...
    print("Step 4: Alerting and monitoring")
    ...
    if alerts:
        ...
        print("  OK All metrics within normal ranges")
...
if __name__ == "__main__":
    ...
    print("Splurge-Tools: Comprehensive Workflow Examples")
    print("=" * 70)
    print()
    
    # Create sample datasets
    temp_dir, files = create_messy_dataset()
    print(f"Created sample datasets in: {temp_dir}")
    print()
    ...
    try:
        ...
        print("• Data Cleaning & Validation: Text normalization + business rules")
        print("• Streaming Analytics: Memory-efficient processing of large datasets")
        print("• Data Transformation: Pivot, group-by, and column transformations")
        print("• ETL Pipeline: Extract, Transform, Load with data enrichment")
        print("• Real-time Processing: Stream validation with live analytics")
        print("\nTECHNOLOGIES COMBINED:")
        print("• DSV parsing with error handling")
        print("• Text processing and normalization")
        print("• Type inference and validation")
        print("• Tabular data models (in-memory and streaming)")
        print("• Data transformation and aggregation")
        print("• Factory pattern for component creation")
        print("• Random data generation for testing")
        print("\nBEST PRACTICES SHOWCASED:")
        print("• Comprehensive error handling at each stage")
        print("• Memory-efficient streaming for large datasets")
        print("• Modular validation with reusable components")
        print("• Data quality checks throughout the pipeline")
