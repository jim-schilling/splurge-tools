...
class TextNormalizer:
    ...
    def normalize_all(
        ...
        *,
        ...
        remove_duplicate_chars: bool = False
    ) -> str:
        ...
        Args:
            ...
            to_ascii: Whether to convert to ASCII
            normalize_quotes: Whether to normalize quotes
            normalize_dashes: Whether to normalize dashes
            normalize_case: Desired case ('lower', 'upper', 'title', 'sentence')
            remove_duplicate_chars: Whether to remove duplicate characters
            
        Returns:
            Normalized string
            
        Example:
            >>> TextNormalizer.normalize_all(
            ...     "Hello  World!",
        ...
        if normalize_whitespace:
            value = cls.normalize_whitespace(value)
            
        if remove_special:
            value = cls.remove_special_chars(value)
            
        if to_ascii:
            value = cls.to_ascii(value)
            
        if normalize_quotes:
            value = cls.normalize_quotes(value)
            
        if normalize_dashes:
            value = cls.normalize_dashes(value)
            
        if normalize_case:
            value = cls.normalize_case(value, case=normalize_case)
            
        if remove_duplicate_chars:
            value = cls.remove_duplicate_chars(value)
